...
/// The part's ORIGINAL on-disk record, verbatim — the 276-byte header plus both still-deflated
/// blobs, exactly as the megapack stores it.
///
/// This is what `bundle`'s `raw/` directory preserves. It is deliberately the compressed form: the
/// guarantee a modder needs is "whatever we failed to decode is still here byte-exact", and that is
/// only true of the bytes we never touched.
pub fn raw_record<'a>(buf: &'a [u8], e: &MeshEntry) -> Option<&'a [u8]> {
    let end = e.file_off + 276 + e.comp0 as usize + e.comp1 as usize;
    (end <= buf.len()).then(|| &buf[e.file_off..end])
}
